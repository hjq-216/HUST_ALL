	...
	if (T)
	{
		...
		case ASSIGN:
			if (T->ptr[0]->kind != ID && T->ptr[0]->kind != ARRAY_DF && T->ptr[0]->kind != ARRAY_CALL) {
				...
				if (T->ptr[0]->type == CHAR && T->kind != ASSIGN) {
					semantic_error(T->pos, T->ptr[0]->type_id, "字符类型变量不能参与运算");
					break;
				}
				if (T->ptr[1]->type == CHAR && T->kind != ASSIGN) {
				...
				else
				{
					...
					strcat(alias, "[");
					//strcat(alias, symbolTable.symbols[T->ptr[1]->place].alias);
					strcat(alias, "]");
				...
				else if(T->ptr[0]->kind == ARRAY_CALL)
				{
					...
					strcat(alias, "[");
					Exp(T->ptr[0]->ptr[1]);
					strcat(alias, symbolTable.symbols[T->ptr[0]->ptr[1]->place].alias);
					strcat(alias, "]");
...
//#endif
		...
		//	T->ptr[1]->type = !strcmp(T->ptr[0]->type_id, "int") ? INT : FLOAT;  //确定变量序列各变量类型
			...
			T0 = T->ptr[1]; //T0为变量名列表子树根指针，对ID、ASSIGN类结点在登记到符号表，作为局部变量
			num = 0;
			T0->offset = T->offset;
			T->width = 0;
			...
			while (T0) {  
				...
				if (T0->ptr[0]->kind == ARRAY_DF) {
					...
					if (rtn == -1)
						...
						Exp(T0->ptr[0]->ptr[1]);
						if(T0->ptr[0]->ptr[1]->kind == ARRAY_DF)
							checkArray(T0->ptr[0]->ptr[1]);
						opn1.kind = ID; 
...
void checkArray(struct ASTNode* T) {
	if (T) {
		switch (T->kind) {
		case INT:
			if (T->type_int <= 0) {
