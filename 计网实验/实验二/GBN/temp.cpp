...
void GBNRdtSender::receive(const Packet &ackPkt) {
    ...
    if(base==nextseqnum) waitingState= false;
        ...
        i=0;
        for(i=0;i<(nextseqnum-base+8)%8;i++){
            pns->stopTimer(SENDER,packetWindow[i].seqnum);
...
void GBNRdtSender::timeoutHandler(int seqNum) {
    ...
    for(int i = 0;i < this->sendnum;i++)
    {
        ...
        pns->stopTimer(SENDER, this->packetWindow[i].seqnum);									//首先关闭定时器
        pns->startTimer(SENDER, Configuration::TIME_OUT, this->packetWindow[i].seqnum);			//重新启动发送方定时器
