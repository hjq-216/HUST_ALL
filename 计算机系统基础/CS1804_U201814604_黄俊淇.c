...
int float_f2i(unsigned uf) {
  	...
  	if (order < 127)// 如果向下溢出
		return 0;
  	if (((uf >> 31) & 1) == 1){// 如果为负数
    	if(order > 150)// 如果阶码超出22，那么尾数左移
      		return ~(x << (order - 150)) + 1;
	else	return ~(x >> (150 - order)) + 1;
    }
  	else
  	{//为正数
    	if(order > 150) //阶码超出22，尾数需左移
			return x << (order - 150);
		return x >> (150 - order);
    }
